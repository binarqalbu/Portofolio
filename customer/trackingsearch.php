...
<html lang="en">
  ...
  <body class="bg-content">
                ...
                <?php include 'database' . '.php'; 
                          //if search condition
                            if(isset($_POST['submitsrc'])){
                              ...
                              $sql ="SELECT * FROM tracking WHERE (raf_id, date) IN (SELECT raf_id, MAX(date) FROM tracking GROUP BY raf_id) and raf_id like '%$search%' and shipper='$shipper' and cusdone <> 'done' order by date desc";
                              $result = mysqli_query($con,$sql);
                            }
                            //not search condition
                            else{
                              $sql = "SELECT * FROM tracking WHERE (raf_id, date) IN (SELECT raf_id, MAX(date) FROM tracking GROUP BY raf_id) and shipper='$shipper' and cusdone <> 'done'  order by date desc";
                              $result = mysqli_query($con,$sql);  
                            }
                          if($result){
                            while($row=mysqli_fetch_assoc($result)){
                              ...
                              echo'<button type="button" class ="btn btn-success btn-sm"><a href="trackingdone?doneid='.$raf_id.'" style="text-decoration:none;" onclick="javascript:confirmationDelete($(this));return false;" class="text-light">Accepted</a></button>';
                              echo'</td>'; 
                              echo'</tr>';
    ...
    {
      var conf = confirm('Are you sure want to done this shipment?');
      if(conf)
